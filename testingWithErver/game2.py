...
class network:
    def __init__(self, host='192.168.1.54', port=8888):
        self.host = host
        self.port = port
...
def main():
    ...
    while run:
        ...
        if len(player.laser_objs) != 0:
            player.move_laser(40, player2)
        if len(player2.laser_objs) != 0:
            player2.move_laser(40, player)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_1:
                    ...
                    run_menu = False
        if player.speed == True or player.damage== True or player.burstfire== True or player.shield== True:
