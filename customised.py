...
class Enemy(Ship):
    def __init__(self,x,y,color,level,health=10):
        self.blueHealth = 10
        self.greenHealth = 20
        self.redHealth = 200
        COLOR_MAP = {'blue': (blue_space_ship, laser_blue,self.blueHealth),
                     'green': (green_space_ship, laser_green,self.greenHealth)
                     }
        super().__init__(x,y,health)
        if not color == 'red':
            self.ship_img,self.laser_img,self.health= COLOR_MAP.get(color)
        else:
            self.ship_img, self.laser_img, self.health = red_space_ship,laser_red,self.redHealth
        self.x =x
        self.y=y
        self.color = color
        # self.vel = velocity
    ...
    def healthBar(self, window):
        if self.health == self.redHealth:
            self.max_health = self.redHealth
        pygame.draw.rect(window, (255, 0, 0),(self.x, self.y - self.ship_img.get_height() - 10, self.ship_img.get_width(), 10))
        pygame.draw.rect(window, (0, 255, 0), (self.x, self.y - self.ship_img.get_height() -10, self.ship_img.get_width() * (self.health / self.max_health),10))
    ...
    while run:
        ...
        if len(enemies) == 0:
            ...
            if level %5==0 :
                ...
                for i in range(wave):
                    enemy = Enemy(random.randrange(70,WIDTH-100),random.randrange(-1000,-100),random.choice(['green','blue']),level)
                    enemies.append(enemy)
        keys = pygame.key.get_pressed()
        if (keys[pygame.K_a] and player.x-player_vel > 0) or(keys[pygame.K_LEFT] and player.x-player_vel > 0):
            player.x -=player_vel
        if (keys[pygame.K_d] and player.x + player_vel + player.get_width()-50< WIDTH) or (keys[pygame.K_RIGHT] and player.x + player_vel + player.get_width()-50< WIDTH):
            player.x +=player_vel
        if (keys[pygame.K_w] and player.y-player_vel >0) or (keys[pygame.K_UP] and player.y-player_vel >0):
            player.y -=player_vel
        if (keys[pygame.K_s] and player.y+player.get_height()+50 +20+player_vel < HEIGHT or (keys[pygame.K_DOWN] and player.y+player.get_height()+50 +20+player_vel < HEIGHT)):
            player.y +=player_vel
        ...
        for enemy in enemies[:]:
            ...
            if boss:
                if random.randrange(1,bossfireRate) ==1 :
                    enemy.shoot()
